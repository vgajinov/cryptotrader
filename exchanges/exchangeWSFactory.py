...
class ExchangeWSFactory(object):
    """
    The Factory class that exposes the names of supported exchanges with the websocket interface to a user
    ...
    def create_client(name, key_file=None) -> WSClientAPI:
        ...
        Note: WSClientAPI is an abstract class implemented by all
              websocket-based exchange client classes
        """
        try:
