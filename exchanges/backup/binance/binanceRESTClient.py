...
from exchanges.binance.helpers import date_to_milliseconds, interval_to_milliseconds
from exchanges.binance.exceptions import BinanceAPIException, BinanceRequestException, BinanceWithdrawException
...
class binanceRESTClient(object):
   ...
   def __init__(self, api_key, api_secret, requests_params=None):
      ...
      self.ping()
      # record the difference in server time and local time
      self.deltaTime = self.get_server_time()['serverTime'] - int(time.time() * 1000)
   ...
   def _generate_signature(self, data):
      print('Generate sig : ', data)
      ordered_data = self._order_params(data)
      query_string = '&'.join(["{}={}".format(d[0], d[1]) for d in ordered_data])
      print('query string : ', query_string)
      m = hmac.new(self.API_SECRET.encode('utf-8'), query_string.encode('utf-8'), hashlib.sha256)
      print('SIG : ', m.hexdigest())
      return m.hexdigest()
   ...
   def _request(self, method, uri, signed, force_params=False, **kwargs):
      ...
      if signed:
         # generate signature
         kwargs['data']['timestamp'] = int(time.time() * 1000) + self.deltaTime
