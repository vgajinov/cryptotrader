...
class BitfinexRESTClient(RESTClientAPI):
    ...
    @response_formatter(fmt.symbols, log)
    def symbols(self):
        return self._public_query('/v1/symbols')

    @response_formatter(fmt.symbols_details, log)
    ...
    @response_formatter(fmt.ticker, log)
    ...
    @response_formatter(fmt.order, log)
    ...
    @response_formatter(fmt.cancel, log)
    ...
    @response_formatter(fmt.forward, log)
    def status(self):
        return self._public_query('/v2/platform/status')

    @response_formatter(fmt.tickers, log)
