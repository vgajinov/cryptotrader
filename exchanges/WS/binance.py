...
class BinanceAllTickers(ChannelData):
    ...
    def update(self, data):
        self.data = []
        for tck in data:
            self.data.append([tck['s']] +
                             [float(tck[key]) for key in ['b', 'B', 'a', 'A', 'p', 'P', 'c', 'v', 'h', 'l']])
        self._publish()
...
class BinanceWSClient(WSClientAPI):
    ...
    def subscribe_order_book(self, symbol, update_handler=None, **kwargs):
        ...
        if self._subscriptions.get(stream, None) is None:
            ...
            if init_book_data is None:
                init_book_data = {'bids': [], 'asks': []}
            self._data[stream] = BinanceOrderBook(stream, init_book_data['bids'], init_book_data['asks'])
            self._subscribe(stream)
        else:
